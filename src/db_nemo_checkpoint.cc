...
class DBNemoCheckpointImpl : public DBNemoCheckpoint {
 public:
  // Creates a DBNemoCheckPoint object to be used for creating openable snapshots
  explicit DBNemoCheckpointImpl(DB* db) : db_(db) {}
  ...
  using DBNemoCheckpoint::CreateCheckpoint;
  virtual Status CreateCheckpoint(const std::string& checkpoint_dir) override;

  using DBNemoCheckpoint::GetCheckpointFiles;
  ...
  using DBNemoCheckpoint::CreateCheckpointWithFiles;
...
Status DBNemoCheckpoint::Create(DB* db, DBNemoCheckpoint** checkpoint_ptr) {
  *checkpoint_ptr = new DBNemoCheckpointImpl(db);
  return Status::OK();
}

Status DBNemoCheckpoint::CreateCheckpoint(const std::string& checkpoint_dir) {
  return Status::NotSupported("");
}

// Builds an openable snapshot of RocksDB
Status DBNemoCheckpointImpl::CreateCheckpoint(const std::string& checkpoint_dir) {
...
Status DBNemoCheckpointImpl::GetCheckpointFiles(std::vector<std::string> &live_files,
    VectorLogPtr &live_wal_files, uint64_t &manifest_file_size,
...
Status DBNemoCheckpointImpl::CreateCheckpointWithFiles(const std::string& checkpoint_dir,
    std::vector<std::string> &live_files, VectorLogPtr &live_wal_files,
